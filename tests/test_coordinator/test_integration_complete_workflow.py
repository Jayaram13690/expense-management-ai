...
class TestCompleteWorkflowIntegration:
    ...
    def test_complete_expense_submission_workflow_integration(self):
        ...
        with patch("coordinator.executor.AgentRequestBuilder.build") as mock_builder:
            ...
            assert (
                result["completed_stages"] == 5
            )  # Should have completed 5 stages before confirmation

            # Verify AgentRequestBuilder was called for each agent stage
            assert (
                mock_builder.call_count == 4
            )  # employee_retrieval, policy_eligibility_lookup, category_limit_lookup, expense_preview
            ...
            expense_agent.invoke.assert_called_once_with(
                "Generate preview for travel expense claim"
            )
    ...
    def test_workflow_resume_integration(self):
        ...
        with patch("coordinator.executor.AgentRequestBuilder.build") as mock_builder:
            mock_builder.side_effect = lambda workflow_step, workflow_context, previous_results: (
                f"Request for {workflow_step.stage_name}"
            )

            employee_agent.invoke.return_value = {"employee_id": "EMP001", "name": "John Doe"}
            policy_agent.invoke.side_effect = [{"policy_id": "POL001"}, {"category_id": "CAT001"}]
